...
# NOW ALSO saves window-level DBSCAN assignments for the best run.
...
def read_ngafid(path: str, cols: List[str], template_dir: str) -> pd.DataFrame:
    ...
    try:
        ...
        if len(templ) >= m:
            ...
            header = templ + [f"Dummy_{i}" for i in range(m - len(templ))]
        raw.columns = header
        try:
            ...
            if missing:
                raise KeyError(
                    f"{os.path.basename(path)}: cannot map {missing} to NGAFID template positions"
                )
            idxs = [NGAFID_POS[c] for c in cols if NGAFID_POS[c] < m]
            if len(idxs) != len(cols):
                raise ValueError(
                    f"{os.path.basename(path)}: file has {m} cols, "
                    f"template indices exceed width for {cols}"
                )
            out = (
                raw.iloc[:, idxs]
                .replace("", np.nan)
...
# ---------- k-distance helpers ----------
def kdist_candidates(
    X: np.ndarray,
    k: int,
    sample: int = 2000,
    percentiles: List[float] = [90, 95, 97, 98, 99, 99.5, 99.7, 99.9],
) -> List[float]:
...
def main():
    ...
    ap.add_argument(
        "--columns",
        nargs="+",
        default=["AltMSL", "E1 RPM", "E1 FFlow", "E1 CHT1", "E1 EGT1", "NormAc", "IAS"],
    )
    ap.add_argument("--window_size", type=int, default=30)
    ap.add_argument("--step_size", type=int, default=25)

    ap.add_argument(
        "--use_pca", action="store_true", help="Apply PCA (retain 95%% var) before DBSCAN."
    )
    ap.add_argument(
        "--pca_var", type=float, default=0.95, help="Variance to retain if --use_pca."
    )
    ap.add_argument(
        "--min_samples_grid",
        nargs="+",
    ...
    assert (
        getattr(scaler, "n_features_in_", None) == 7
    ), "Scaler must be 7-feature per-timestep."
    ...
    for ms in args.min_samples_grid:
        ...
        print(
            f"[grid] min_samples={ms}, eps candidates ~ "
            f"{eps_list[:6]}{'...' if len(eps_list)>6 else ''}"
        )
    ...
    if best is None or best["clusters"] == 0:
        raise SystemExit(
            "DBSCAN still found no usable clusters. "
            "Try --use_pca and/or add --extra_eps 2.0 3.0"
        )

    # -------------------------------------------------------
    # NEW: save window-level DBSCAN assignments for best run
    # -------------------------------------------------------
    best_labels = best["labels"]
    assign_df = pd.DataFrame(
        {
            "window_idx": np.arange(len(best_labels)),
            "dbscan_label": best_labels,
        }
    )
    assign_path = os.path.join(
        args.out_dir,
        f"assignments_dbscan_eps{best['eps']}_min{best['min_samples']}.csv",
    )
    assign_df.to_csv(assign_path, index=False)
    print(f"[INFO] Saved DBSCAN window assignments → {assign_path}")
    ...
    print(
        f"Best: eps={best['eps']}  min_samples={best['min_samples']}  "
        f"clusters={best['clusters']}  noise={best['noise_rate']:.2f}  "
