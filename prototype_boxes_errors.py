...
def parse_window_idx(basename: str) -> int:
    m = re.search(r"window_(\d+)\.csv$", basename)
    return int(m.group(1)) if m else -1

def read_error_window_vector(path: str, columns: List[str]) -> np.ndarray:
    # enforce feature order and drop anomaly_flag
    df = pd.read_csv(path)
    df = df[columns]
...
def fit_prototypes_on_errors(
    ...
    X_list = []
    for f in files:
        v = read_error_window_vector(f, columns)
        X_list.append(v)
...
def explain_error_folder(
    ...
    for f in files:
        ...
        widx = parse_window_idx(base)
        x = read_error_window_vector(f, columns)
        if scaler is not None:
            x = scaler.transform(x.reshape(1, -1)).ravel()
        if x.shape[0] != D_expected:
            print(f"[WARN] {base}: vector dim {x.shape[0]} != expected {D_expected}; skipping")
            continue
        ...
        row = {
            "file": base,
            "window_idx": int(widx),            # <-- needed by visualizer
            "prototype_id": k_id,
    ...
    out_df = pd.DataFrame(rows).sort_values(["file", "window_idx"]).reset_index(drop=True)
